...
> 在接力模式（或任何预设）中打开本仓库工作时，本文件即权威协议。
> 违反"红线"或"版本铁律"的改动视为失败提交。

## 1. 本仓库是什么

DeepSeek Harness 的上下文接力能力栈：预设 `continuity`（接力模式）+ 三个 host 驱动器，全部用户自有、位于 `~/.dsh` 下、与业务仓库解耦。`deploy/` 是**真实运行文件**的镜像——运行位置由 harness 固定（见 §8），仓库通过镜像进行版本控制。
