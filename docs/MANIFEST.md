...
| 组件 | 当前版本 | 真实位置 | 被引用处 |
|---|---|---|---|
| 预设插件 | **v29** | `~/.dsh/.agent-presets/continuity/continuity-plugin.v29.mjs` | `agent.cordis.yml`（continuity group 内） |
...
旧代次（插件 v1..v28、rotation base/v2..v6、worktree v1..v6、mission v1..v6）**全部保留**——运行中的进程可能仍引用它们，禁止删除。
...
```powershell
node tests\continuity-unit-tests.mjs      # 73 项
node tests\continuity-rotation-tests.mjs  # 20 项
node tests\continuity-worktree-tests.mjs  # 32 项
node tests\continuity-mission-tests.mjs   # 23 项  （合计 148）
