...
| 测试设施 | `~/.dsh/designs/continuity-v3/tests/`（31 项为 V3 MVP 历史基线；当前 **145 项全绿**：unit 70 + rotation 20 + worktree 32 + mission 23，见 §7d/§7e）+ opt-scratch/smoke-a/smoke-b 草稿工作区 | 可扩展 |
...
**跨代引用（A.8，G7 连带）**：三个 host 驱动器（rotation v7 / worktree v7 / mission v7）现统一从 `continuity-shared.v1.mjs`（绝对 file URL，`~/.dsh/continuity-host/`）导入纯 helper（MARKER/validateCheckpoint/userMessage/capText/textOfMessage/sectionBody/buildInstruction）——**checkpoint 格式单一真相源**；MISSION_MARKER 亦全部从 shared.v1 导入（mission v6/v7、worktree v7 收敛完成，worktree v7 再导出保持兼容）。活动预设挂 v27（`/sessions_active`、`/current_session`、有界 hub onboarding、`--` 用户想法、终局回复转发、peek 去重、`/steer` 重要升级、模式改名 Orchestra、`/mission stop` + 裸 `/status`、**协调链接持久化 + 空闲巡检** 见 §7e）。checkpoint 格式三代稳定；升级 checkpoint 格式时需同步 shared.v1 一处即可。
...
- 145 项单元测试全绿（unit 70 + rotation 20 + worktree 32 + mission 23）；基线 141 为上一代快照。
...
- 回归：MVP 31 项单元测试（V3 历史基线）；当前 145 项全绿（unit 70 + rotation 20 + worktree 32 + mission 23，见 §7d/§7e）；`/handoff` `/continue` 行为不变。
