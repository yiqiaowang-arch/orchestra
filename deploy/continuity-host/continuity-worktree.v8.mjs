 ...
 *
 * v7 (this file): MISSION_MARKER convergence — the mission checkpoint marker
 * is now IMPORTED from `continuity-shared.v1.mjs` (the single source of
 * truth) and re-exported for compatibility, instead of being defined
 * locally. Value unchanged (`<!-- DSH_MISSION v1 -->`); consumers importing
 * MISSION_MARKER from this driver keep working.
 *
 * v6 (this file, EXPERIMENTAL): forward worker approval (elevation) requests
 * to the coordinator via `ctx.userQuestions` — the human decides from the MAIN
 * conversation (`forwardWorkerApprovals`, default true; falls back to the
 * in-worker prompt on failure/timeout). Also keeps v5's worker-completion
 * notice and v3's error-path hardening.
 *
 * v5 (worker completion notification): when a spawned worker posts
...
// checkpoint marker (mission v6/v7 imports the same binding from shared).
export { MISSION_MARKER }
...
const DEFAULTS = Object.freeze({
  ...
  forwardWorkerApprovals: true, // forward worker approval requests to the coordinator via userQuestions (experimental)
  approvalForwardTimeoutMs: 120000, // fall back to the in-worker prompt when the coordinator card is unanswered
})
...
export function sanitizeWorktreeConfig(raw) {
  ...
  return {
    ...
    forwardWorkerApprovals: src.forwardWorkerApprovals !== false,
    approvalForwardTimeoutMs: intInRange(src.approvalForwardTimeoutMs, DEFAULTS.approvalForwardTimeoutMs, 5000, 600000),
  }
}

/**
 * Map a user-questions answer onto an approval outcome (pure, testable).
 * Returns 'allowed-once' | 'rejected' | null (null = not decided → caller falls
 * through to the normal approval path). `answer` is the raw ask() result.
 */
export function approvalOutcomeOfAnswer(answer) {
  ...
  if (selected.includes('批准（allowed-once）')) return 'allowed-once'
  if (selected.includes('拒绝')) return 'rejected'
  return null
}
    ...
    try {
      ...
      await agents.create({
        ...
        setup: async (agentCtx) => {
          ...
          // v6 (experimental): forward this worker's approval requests to the
          // coordinator as a user-questions card, so the human can decide from
          // the MAIN conversation. Registered prepended on the worker scope so
          // a decided answer short-circuits before the in-worker browser card;
          // on any failure or timeout it falls through to next() (the normal
          // in-worker prompt). The worker's tool call waits for the answer.
          if (cfg.forwardWorkerApprovals) {
            agentCtx.on('approval/request', async (req, next) => {
              const uq = get('userQuestions')
              const agents = get('agents')
              const live = agents !== undefined ? agents.get(coordinator.id) : undefined
              if (uq === undefined || live === undefined || live !== coordinator) return next()
              if (req.signal !== undefined && req.signal.aborted) return next()
              const controller = new AbortController()
              const onAbort = () => controller.abort()
              if (req.signal !== undefined) req.signal.addEventListener('abort', onAbort, { once: true })
              const timer = setTimeout(() => controller.abort(), cfg.approvalForwardTimeoutMs)
              try {
                const answer = await uq.ask({
                  questions: [{
                    id: 'continuity-worker-approval',
                    header: 'Worker 提权请求',
                    question: 'Worker ' + workerId + '（' + (title !== '' ? title : brief) + '）请求批准 '
                      + (req.toolName !== undefined ? req.toolName : '工具操作')
                      + (req.reason !== undefined ? '：' + req.reason : '')
                      + '。批准仅对本次操作生效（allowed-once）。',
                    options: [
                      { label: '批准（allowed-once）', description: '仅放行本次操作' },
                      { label: '拒绝', description: '本次操作按未授权处理' },
                    ],
                  }],
                  agent: live,
                  signal: controller.signal,
                })
                const outcome = approvalOutcomeOfAnswer(answer)
