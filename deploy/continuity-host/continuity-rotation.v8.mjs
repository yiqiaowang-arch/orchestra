 ...
 *
 * v8 (this file): COORDINATION LINKS MIGRATE WITH THE ROTATION — a rotated
 * hub keeps its organisational context: after injecting the snapshot, the
 * driver scans the source session's durable log for the newest
 * `<!-- DSH_COORD_LINKS v1 -->` record, rewrites `hub=` to the successor,
 * injects the record into the successor, and injects a fresh `hub=<new>`
 * record into every spoke (via the agents registry). After a restart the
 * plugin's restore picks the NEWEST record, so the successor automatically
 * resumes as the hub and spokes forward to it — no manual /coordinate-hub
 * and no double-forwarding. `/worker-successor` inherits the worker's link
 * record unchanged (the worker keeps forwarding to the same hub). Same
 * command surface. Shared import URL v1 → v2.
 *
 * v7 (this file): the rotation successor is attached to the source session's
 * workspace (`workspaceRegistry.attachSession`) right after creation, so it is
 * grouped under that workspace in the GUI instead of falling into Ungrouped.
 *
 * v6 (this file): an invalid checkpoint attempt (marker present, sections
 * missing) no longer aborts /rotate immediately — the rollover waiter keeps
 * waiting for a valid checkpoint, bounded by maxWaitMs.
 *
...
export default function continuityRotation(ctx, config) {
  ...
  /**
   * v8: migrate coordination links onto a successor. Scans the source
   * session's durable log for the newest COORD_LINK_MARKER record, injects
   * it into the successor (with `hub=` rewritten to the successor when
   * requested), and points every spoke at the successor with a fresh
   * `hub=<successor>` record — the plugin's restore picks the NEWEST record
   * after a restart, so the successor resumes as the hub and spokes forward
   * to it without manual re-linking or double-forwarding. Best-effort: any
   * failure leaves the successor without links (the user can still run
   * /coordinate-hub manually). Returns true when a record was migrated.
   */
  async function migrateCoordinationLinks(next, sourceId, sessionQuery, agentsSvc, rewriteHubTo) {
    if (next === null || next === undefined || next.session === undefined) return false
    if (sessionQuery === undefined || typeof sessionQuery.readSession !== 'function') return false
    const log = await sessionQuery.readSession(sourceId)
    ...
    for (let index = events.length - 1; index >= 0; index -= 1) {
      ...
      recordText = text
      const spokesMatch = /(?:^|\s)spokes=(\S+)/.exec(text)
      if (spokesMatch !== null) spokes = spokesMatch[1].split(',').filter((s) => s !== '')
      break
    }
    if (recordText === null) return false
    const rewritten = (rewriteHubTo !== null && rewriteHubTo !== undefined)
      ? recordText.replace(/(?:^|\s)hub=\S+/, ' hub=' + rewriteHubTo)
      : recordText
    try { next.inject(userMessage(rewritten, 'continuity-links')) } catch { /* best-effort */ }
    for (const spokeId of spokes) {
      try {
        const spoke = agentsSvc !== undefined ? agentsSvc.get(spokeId) : undefined
        if (spoke !== undefined && typeof spoke.inject === 'function') {
          spoke.inject(userMessage(COORD_LINK_MARKER + ' hub=' + next.id, 'continuity-links'))
        }
  ...
  async function rotate(agent, signal) {
    ...
    try {
      ...
      // v7: attach the successor to the source session's workspace so the GUI
      // groups it under that workspace (mirrors the worktree driver's
      // attachSession). Best-effort: an attach failure only loses grouping.
      if (cwd !== undefined) {
        const workspaces = get('workspaceRegistry')
        if (workspaces !== undefined) {
          try {
            const ws = await workspaces.resolveByPath(cwd)
            if (ws !== undefined) await ws.attachSession(newId)
          } catch { /* grouping is advisory */ }
        }
      }
      ...
      // 3b. v8: migrate coordination links so the successor resumes as the hub
      // after a restart (no manual /coordinate-hub, no double-forwarding).
      let linkMigrated = false
      try {
        linkMigrated = await migrateCoordinationLinks(next, session.id, get('sessionQuery'), get('agents'), newId)
      } catch { /* best-effort */ }
      ...
      return {
        ...
        text: 'Rollover complete: continuation session ' + newId + ' created in the same workspace and woken'
          ...
          + (linkMigrated ? '; coordination links migrated to the successor (spokes will forward to it after a restart)' : '')
  ...
  async function rotateSuccessor(coordinator, workerId, instruction, signal) {
    ...
    try {
      ...
      // v8: the successor worker inherits the worker's coordination links
      // unchanged (it keeps forwarding to the same hub).
      try {
        await migrateCoordinationLinks(next, workerIdClean, get('sessionQuery'), get('agents'), null)
      } catch { /* best-effort */ }
      next.followup({
  ...
  function onSessionEvent(session, event) {
    ...
    // v6: only a VALID checkpoint settles the rollover waiter. An invalid
    // marker-bearing attempt (missing sections, retry in progress) must NOT
    // abort /rotate immediately — the waiter keeps waiting for a valid
    // checkpoint, bounded by maxWaitMs (previously the first invalid attempt
    // aborted the rollover with "missing required section(s)").
    if (!verdict.ok) return
    resolveCheckpointWaiters(session.id, true, null, event.seq)
  }
