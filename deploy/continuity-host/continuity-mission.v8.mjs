 ...
 *
 * v8 (this file): shared import URL v1 → v2 (COORD_LINK_MARKER added there);
 * otherwise byte-for-byte identical to v7 — no behavior change.
 *
 * v7 (this file): `/mission stop` — the user's explicit cancel. A running
 * mission (planning / dispatching / closing) can be cancelled: pending
 * waiters and their timeouts are cleared (the in-flight loop aborts
 * silently at the next stage boundary), the phase settles as `cancelled`
 * (a terminal phase like done/failed — resume rejects it, start allows a
 * fresh mission), and the reply notes that already-spawned worktree
 * workers keep running and are stopped individually with /worker-stop.
 * Stop is in-process state: after a process restart the durable
 * checkpoint may still allow /mission resume (same as a failed mission).
 *
...
import { MISSION_MARKER } from 'file:///C:/Users/wangy/.dsh/continuity-host/continuity-shared.v2.mjs'
...
export function missionSummary(state, elapsed) {
  ...
  switch (state.phase) {
    ...
    case 'cancelled':
      return '已取消 🛑' + when + '（/mission stop）。可以发 /mission <目标> 重新开始。'
    default:
...
export default function continuityMission(ctx, config) {
  ...
  function freshState() {
    return {
      phase: 'idle', // idle | planning | dispatching | closing | done | failed | cancelled
      goal: '',
      tasks: [],
      results: [],
  ...
  /**
   * v7: cancel every pending waiter of a session — marks them done, runs
   * their timer cancel (native-setTimeout path), settles the pending
   * promise with null, and drops the table entry. Used by /mission stop so
   * the in-flight loop's current wait resolves immediately instead of
  ...
  function cancelWaiters(sessionId) {
    ...
    for (const waiter of entry) {
      ...
      waiter.done = true
      if (waiter.cancel !== null) waiter.cancel()
      if (waiter.settle !== null) waiter.settle(null)
    }
    waits.delete(sessionId)
  }
  ...
  async function escalate(coordinator, state, reason) {
    // v7: /mission stop aborts the loop silently — no failure steer, no phase overwrite.
   ...
   */
  async function reviewOutcome(coordinator, state, outcome) {
    ...
    let rounds = 0
    while (true) {
      // v7: /mission stop aborts the review loop — no new waits after a cancel.
      if (state.phase === 'cancelled') {
        return { task, spawn, report, verdict: null, reviewError: 'cancelled', rounds }
      }
      let verdict = null
  ...
  async function runMission(coordinator) {
    ...
    try {
      ...
      let planned = null
      for (let attempt = 0; attempt < 2 && planned === null && state.phase !== 'cancelled'; attempt += 1) {
      ...
      if (planned === null) {
        // v7: a cancelled mission returns silently instead of escalating.
      ...
      let index = 0
      while (index < state.tasks.length && state.phase !== 'cancelled') {
        ...
        for (const outcome of collected) {
          if (state.phase === 'cancelled') return
          state.results.push(await reviewOutcome(coordinator, state, outcome))
        }
      ...
      // v7: a cancelled mission never falls into `failed`.
      if (state.phase !== 'cancelled') {
        state.phase = 'failed'
  ...
  async function resume(coordinator) {
    ...
    if (state.phase === 'cancelled') {
      return error('The mission was cancelled with /mission stop; start a new one with /mission <goal>.')
    }
  ...
  async function start(coordinator, goal) {
    ...
    if (trimmed === '') return error('Usage: /mission <goal> — or /mission resume, /mission status, /mission stop')
    if (trimmed.toLowerCase() === 'resume') return await resume(coordinator)
    const state = getState(coordinator.id)
    if (state.phase !== 'idle' && state.phase !== 'done' && state.phase !== 'failed' && state.phase !== 'cancelled') {
      return error('A mission is already running (phase ' + state.phase + '); inspect it with /mission status.')
    }
    clearWaiters(coordinator.id)
    Object.assign(state, freshState())
  ...
  /**
   * v7: /mission stop — cancel a running mission. Clears every pending waiter
   * (its timer and promise), settles the phase as `cancelled`, and lets the
   * in-flight loop abort silently at the next stage boundary. Already-spawned
   * worktree workers are NOT touched — they keep running and are stopped
   * individually with /worker-stop.
   */
  function stop(coordinator) {
    const state = getState(coordinator.id)
    if (state.phase !== 'planning' && state.phase !== 'dispatching' && state.phase !== 'closing') {
      return error('No running mission to stop (phase ' + state.phase + '). Start one with /mission <goal>.')
    }
    state.phase = 'cancelled'
    state.error = 'cancelled by /mission stop'
    cancelWaiters(coordinator.id)
    return success('Mission cancelled by /mission stop. Pending waits and timers cleared; already-spawned workers keep running (stop them individually with /worker-stop <worker-id>). Start a fresh mission with /mission <goal>.')
  }
