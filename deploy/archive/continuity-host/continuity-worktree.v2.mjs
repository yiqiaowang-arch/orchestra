    ...
    try {
      ...
      if (gitWorktree) {
        ...
        if (existsSync(plan.path)) {
          return { ok: false, error: 'Worker directory already exists: ' + plan.path }
        }
        try {
      ...
      if (worker === undefined) return { ok: false, error: 'worker session was created but not found in the registry' }
      ...
      return { ok: true, workerId, path: plan.path, branch: plan.branch, git: gitWorktree }
    } catch (spawnError) {
      return { ok: false, error: '/worktree failed: ' + fail(spawnError) }
    }
  }
