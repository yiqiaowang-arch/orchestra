...
/** v13: steered into the hub session when /coordinate-hub links spokes. */
function coordinatorOnboardingPrompt(hubId, spokes) {
  return [
    ...
    '',
    'Begin now with the intake:',
    '1. Ask EACH spoke about its current task, progress and blocking problems — send one question per spoke with: /relay <spoke-id> <question>',
    '2. Their replies auto-forward here. Read them and synthesize a shared picture: what each side is doing, their contexts, where they conflict, what each needs.',
    '3. Then propose a joint plan: what each spoke should do next, what to coordinate, and how conflicts are resolved. Direct each via /relay. Use /session-peek <spoke-id> for detail before deciding.',
    '4. Keep messages to spokes short and concrete. If a spoke reports something risky or unsafe, say so and propose a safer path.',
    '5. ONE DELEGATION MECHANISM PER TASK (v16): never run the same task through BOTH a worktree worker AND a subagent. If a worktree worker already covers a task, extend it with /worker-send <worker-id> <message>; use the subagent tool only for genuinely new, independent tasks. When a task already has a worker, prefer pushing to that worker over spawning anything new.',
    '',
...
export default function continuityPlugin(ctx, config) {
  ...
  async function handleCoordinateHub(invocation) {
    ...
    const raw = (typeof invocation.rawInput === 'string' ? invocation.rawInput : '').trim()
    if (raw === '') {
      return { kind: 'error', text: 'Usage: /coordinate-hub <spoke-session-id> [spoke-session-id ...] — this session becomes the coordinator; each spoke auto-forwards its replies here; you direct spokes with /relay and inspect them with /session-peek.' }
    }
    const agentsSvc = ctx.get('agents')
    if (agentsSvc === undefined) return { kind: 'error', text: 'Agent registry unavailable.' }
    const linked = []
    for (const spoke of raw.split(/\s+/)) {
    ...
    try {
      agent.steer(userMessage(coordinatorOnboardingPrompt(agent.id, linked), 'continuity-coord-hub'))
    } catch { /* onboarding steer is best-effort */ }
    return {
      kind: 'success',
      text: 'Coordination hub active. Spokes (auto-forward to you): ' + linked.join(', ')
        + '.\nOnboarding steered: you will now ask each spoke about its task and problems, then mediate.\n'
  ...
  async function handleSessionPeek(invocation) {
    ...
    if (targetId === '') return { kind: 'error', text: 'Usage: /session-peek <session-id> [limit]' }
    let limit = 5
    if (parts.length > 1) {
  ...
  const CONTINUITY_ROLES_SECTION = [
    ...
    '- Coordinator (hub) discipline: when coordinating existing sessions, understand EACH session own context before deciding; ask them about their tasks and problems first (onboarding / /coordinate-intake); mediate like a lead toward a constructive, safe, creative joint solution; never force unilateral changes; keep messages short and concrete.',
  ].join('\n')
  ...
  ctx.effect(() => {
    ...
    if (commands !== undefined) {
      ...
      disposers.push(commands.register({
        name: 'session-peek',
        description: 'Read the latest messages of another session (read-only; works on any existing session).',
