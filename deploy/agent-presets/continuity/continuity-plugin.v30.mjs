...
/** v27: idle-hub check-in prompt — re-activate the coordinator on silent stretches. */
export function hubCheckPrompt(idleMinutes, hubId) {
  return [
...
export default function continuityPlugin(ctx, config) {
  ...
  function maybeHubCheck(agent) {
    ...
    hubChecks.set(agent.id, record)
    try {
      agent.steer(userMessage(hubCheckPrompt(idleMin, agent.id), 'continuity-hub-check'))
    } catch { /* best-effort */ }
  }
  ...
  function hubTick() {
    ...
    for (const hubId of [...coordHubs.keys()]) {
      ...
      if (agent !== undefined) maybeHubCheck(agent)
    }
  }
