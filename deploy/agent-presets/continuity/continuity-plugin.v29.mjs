...
function coordinatorOnboardingPrompt(hubId, spokes, note, marker) {
  ...
  const markerBlock = hasMarker
    ? ['', 'FORWARD MARKER (v28): this deployment only auto-forwards messages that contain "' + marker + '". Tell EACH spoke: prefix "' + marker + '" when you want the coordinator to intervene; otherwise the coordinator checks in periodically and stays quiet.', '']
    : []
...
export default function continuityPlugin(ctx, config) {
  ...
  function onCoordEvent(session, event) {
    ...
    if (cfg.coordinateForwardMarker !== '' && !text.includes(cfg.coordinateForwardMarker)) return
  ...
  // v29: the forward-marker protocol lives in the roles section too, so every
  // session (spokes included) knows it from its own system prompt — no
  // coordinator relay needed, applies automatically after a restart.
  const FORWARD_MARKER_ROLES_LINE = cfg.coordinateForwardMarker !== ''
    ? '- ALL ROLES — forward marker protocol (v29): this deployment auto-forwards only messages containing "' + cfg.coordinateForwardMarker + '". Prefix "' + cfg.coordinateForwardMarker + '" to your FINAL reply when you want the coordinator to intervene; otherwise the coordinator stays quiet and checks in periodically.'
    : null
