...
function coordinatorOnboardingPrompt(hubId, spokes) {
  return [
    ...
    '5. ONE DELEGATION MECHANISM PER TASK (v16): never run the same task through BOTH a worktree worker AND a subagent. If a worktree worker already covers a task, extend it with /worker-send <worker-id> <message> instead of spawning anything new.',
    '6. HOW TO DELEGATE (v17): pick the tool by what the task needs. /worktree worker = the task needs a CLEAN, isolated workspace (file writes, long-running, separate directory). Subagent tool = the task does NOT need a clean workspace (it can share this cwd) but would POLLUTE this conversation context (research, searches, noisy tool output, many intermediate steps). Do it yourself only for quick, low-noise, no-isolation work. A subagent shares the workspace but keeps the coordinator context clean.',
    '',
