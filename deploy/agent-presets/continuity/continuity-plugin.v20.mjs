...
function coordinatorOnboardingPrompt(hubId, spokes) {
  return [
    ...
    '1. /session-peek EACH spoke (one peek each, limit ~10) to see its context.',
    ...
    '4. THEN STOP. Report the shared picture to the user and WAIT for their reply. Do NOT propose a joint plan, do NOT direct spokes with /relay, do NOT start content work, until the user responds. A new user message is the user\'s own thought/directive — fold it in, then mediate and direct.',
    '',
...
export default function continuityPlugin(ctx, config) {
  ...
  const CONTINUITY_ROLES_SECTION = [
    ...
    '- Coordinator (hub) discipline: when coordinating existing sessions, understand EACH session own context before deciding (spokes are SIBLING sessions, NOT subagents — inspect with /session-peek, never list_agents or the subagent registry); ask them about their tasks and problems first (onboarding / /coordinate-intake); after the intake round STOP and report to the user, and WAIT for the user\'s own thoughts before directing spokes; mediate like a lead toward a constructive, safe, creative joint solution; never force unilateral changes; keep messages short and concrete.',
  ].join('\n')
