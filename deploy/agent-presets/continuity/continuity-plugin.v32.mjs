 ...
 *
 * v32 (this file): COORD_LINK_MARKER CONVERGED — the durable coordination
 * record marker is now IMPORTED from `continuity-shared.v2.mjs` (single
 * source of truth, alongside MARKER/MISSION_MARKER) and re-exported for
 * compatibility; rotation v8 uses the same binding to migrate links onto a
 * rotated successor. Value unchanged. Same command surface (25).
 *
 * v31 (this file): PRESSURE-AWARE HUB CHECK-IN — the periodic check-in now
 * ALSO measures every spoke's context pressure (tokenMeter + llm capacity,
 * same math as /continuity) and appends a CONTEXT PRESSURE ALERT for each
 * spoke past the rotate threshold (`rotateRatio`): workers get the
 * /worker-successor hint, other spokes get a /rotate suggestion for the
 * user. Spokes with unknown capacity are skipped; a non-live spoke is
 * reported. The coordinator still decides — the check-in just surfaces the
 * pressure so "下属 context 满" is never silent. Same command surface (25).
 *
 * v30 (this file): COMPLETION REPORTS ALWAYS FORWARD — a spoke that finishes
 * but sees "nothing to coordinate" must still surface its DONE state, or the
 * hub only notices at the next periodic check-in. Two changes: (a) the
 * forward gate now ALSO passes any turn-final message containing
 * "## Worker report" (the stack's established completion convention), so a
 * finished spoke reaches the hub even without the coordination marker;
 * (b) the roles-section protocol now tells every session to ALWAYS end a
 * finished task with the marker (or "## Worker report") even when nothing
 * needs coordination. The hub then wakes, reads, and stays quiet if truly
 * nothing needs it. Same command surface (25).
 *
 * v29 (this file): MARKER PROTOCOL IN THE ROLES SECTION — the forward-marker
 * protocol is no longer relayed only through hub onboarding: when
 * `coordinateForwardMarker` is configured, the roles section (present in
 * EVERY session's system prompt, spokes included) carries the instruction,
 * so after a restart the protocol applies automatically with no need to
 * re-run /coordinate-hub. Same command surface (25).
 *
 * v28 (this file): FORWARD MARKER GATE + FULL-LOG RESTORE FALLBACK —
 * (a) new config `coordinateForwardMarker` (default '' = current behavior:
 * every turn-final message forwards). When set (e.g. "请coordinate以下消息"),
 * only turn-final messages CONTAINING the marker are auto-forwarded: spokes
 * flag what needs the coordinator, everything else stays silent and is
 * covered by the periodic hub check-in — the coordinator's context gets
 * exactly the messages worth acting on, with no ack chatter. Applies to
 * `/coordinate` pairs and `/coordinate-hub` spokes alike (same filter).
 * (b) durable-link restore gets a full-log fallback: when the session's
 * in-memory events no longer carry the link record (e.g. compacted away),
 * `sessionQuery.readSession` re-scans the durable log and rebuilds the
 * links for the next event. Same command surface (25).
 *
 * v27 (this file): DURABLE COORDINATION LINKS + IDLE HUB CHECK-IN — the
 * goal is a coordinator that keeps the ensemble working unattended.
 * (a) Links survive restarts: `/coordinate` and `/coordinate-hub` now
 * inject a durable link record (`<!-- DSH_COORD_LINKS v1 -->`,
 * `hub=<id> spokes=…` / `peers=<id>`) into the involved sessions' logs;
 * after a process restart the plugin lazily re-reads the newest record and
 * rebuilds `coordLinks`/`coordHubs` (restore-once per session). (b) Idle
 * re-activation: a hub with spokes that stays silent for `hubCheckMinutes`
 * (default 15) gets a light "Coordinator check-in" steer — peek each spoke
 * (dedupe-aware), mediate finished/stalled ones, otherwise reply in one
 * line and stay quiet. User activity and forwarded messages reset the
 * window; busy agents are never interrupted; the ticking uses the host
 * timer service when present (tests run event-driven only). Same command
 * surface (25).
 *
 * v26 (this file): MISSION STOP + BARE /STATUS — `/mission stop` cancels a
 * running mission (delegated to host mission v7: clears waits/timers,
 * settles phase `cancelled`, already-spawned workers keep running); the
 * bare `/status` command becomes a space-free alias of `/mission status`
 * alongside `/mission_status`. Command surface: 25.
 *
 ...
 *
 * The module is dependency-free ESM: since v32 it imports exactly one
 * absolute-file-URL constant (COORD_LINK_MARKER from continuity-shared.v2),
 * so it still loads from any user directory without a node_modules walk. The default export is the
...
/**
 * v32: single-source imports — COORD_LINK_MARKER comes from
...
/** v32: converged into continuity-shared.v2 (single source of truth); re-exported for compatibility. */
export { COORD_LINK_MARKER }

/** v30: the stack's established completion convention — always surfaced to the hub. */
export const WORKER_REPORT_MARKER = '## Worker report'

/**
 * v27 pure: parse a durable link record.
 * Formats (one marker line, rest of the message ignored):
 *   `hub=<hubId> spokes=<id1>,<id2>`  — hub session + its spokes
 *   `hub=<hubId>`                     — a spoke forwarding to its hub
 *   `peers=<otherId>`                 — one side of a /coordinate pair
 * Returns { hub, spokes } | { hub } | { peers } | null.
 */
export function parseLinkRecord(text) {
  if (typeof text !== 'string' || !text.includes(COORD_LINK_MARKER)) return null
  const line = text.split(/\r?\n/).find((l) => l.includes(COORD_LINK_MARKER))
  if (line === undefined) return null
  const body = line.slice(line.indexOf(COORD_LINK_MARKER) + COORD_LINK_MARKER.length).trim()
  const hub = /(?:^|\s)hub=(\S*)/.exec(body)
  const spokes = /(?:^|\s)spokes=(\S*)/.exec(body)
  const peers = /(?:^|\s)peers=(\S*)/.exec(body)
  if (hub !== null) {
    const record = { hub: hub[1] }
    if (spokes !== null && spokes[1] !== '') record.spokes = spokes[1].split(',').filter((s) => s !== '')
    return record
  }
  if (peers !== null) {
    return peers[1] === '' ? { peers: '' } : { peers: peers[1] }
  }
  return null
}

/**
 * v27 pure gate: whether an idle hub check-in is due.
 * `record` is the per-session { lastCheckAt, baselineUserSeq } (may be null).
 */
export function hubCheckDue(record, cfg, now) {
  if (record === null || record === undefined) return false
  if (record.lastCheckAt === null || record.lastCheckAt === undefined) return false
  if (cfg === null || cfg === undefined || typeof cfg.hubCheckMinutes !== 'number') return false
  return now - record.lastCheckAt >= cfg.hubCheckMinutes * 60000
}
...
const DEFAULT_CONFIG = Object.freeze({
  ...
  hubCheckMinutes: 15, // idle hub re-activation: check-in steer after this many silent minutes
  coordinateForwardMarker: '', // v28: only forward turn-final messages containing this marker ('' = forward all)
})
...
export function sanitizeConfig(raw) {
  ...
  const config = {
    ...
    hubCheckMinutes: intInRange(src.hubCheckMinutes, DEFAULT_CONFIG.hubCheckMinutes, 1, 120),
    coordinateForwardMarker: typeof src.coordinateForwardMarker === 'string'
      ? src.coordinateForwardMarker.slice(0, 64)
      : '',
  }
...
/** v13: steered into the hub session when /coordinate-hub links spokes. v21: note = the user's thoughts (after `--`). v28: marker = forward-marker gate. */
function coordinatorOnboardingPrompt(hubId, spokes, note, marker) {
  const hasNote = typeof note === 'string' && note.trim() !== ''
  const hasMarker = typeof marker === 'string' && marker !== ''
  const userThoughts = hasNote
    ...
    : []
  const markerBlock = hasMarker
    ? ['', 'FORWARD MARKER (v28/v30): this deployment only auto-forwards messages that contain "' + marker + '" (or "## Worker report"). Tell EACH spoke: prefix "' + marker + '" when you want the coordinator to intervene, and ALWAYS end a finished task with "' + marker + '" (or "## Worker report") even when nothing needs coordination — so the coordinator knows it is done; otherwise the coordinator checks in periodically and stays quiet.', '']
    : []
...
/** v27: idle-hub check-in prompt — re-activate the coordinator on silent stretches. v31: alerts = context-pressure alerts. */
export function hubCheckPrompt(idleMinutes, hubId, alerts) {
  const alertBlock = Array.isArray(alerts) && alerts.length > 0
    ? ['', 'CONTEXT PRESSURE ALERTS (v31):', ...alerts.map((alert) => '- ' + alert), 'Address each alert only if it needs action (workers: /worker-successor after their final checkpoint; others: tell the user to /rotate them).', '']
    : []
  return [
    'Coordinator check-in (continuity).',
    'You have been waiting about ' + String(idleMinutes) + ' minutes while coordinating spokes from session ' + hubId + '.',
    ...alertBlock,
    'Before staying quiet:',
    '1. /session-peek each spoke (messages already auto-forwarded to you are skipped).',
    '2. If any spoke finished or is stalled, mediate: /relay a short directive, or report to the user.',
    '3. If everything is progressing normally, reply in ONE line (e.g. "一切正常") and stay quiet.',
    'Do NOT start new work, do NOT steer a busy spoke, do NOT duplicate an in-flight mission, and keep it short.',
  ].join('\n')
}

/**
 * v31 pure: context-pressure alert for one spoke, or null when under the
 * threshold or when capacity is unknown. Same math as /continuity.
 */
export function spokePressureAlert(spokeId, totalTokens, capacity, rotateRatio) {
  ...
  if (ratio < rotateRatio) return null
  const pct = Math.round(ratio * 100)
  return spokeId + ': context at ' + pct + '% (past the rotate threshold ' + Math.round(rotateRatio * 100) + '%) — if it is a worktree worker, run /worker-successor ' + spokeId + ' after its final checkpoint; otherwise suggest the user /rotate it'
}
...
export default function continuityPlugin(ctx, config) {
  ...
  // v27: durable-link restore-once guard + idle-hub check-in state.
  const restoredSessions = new Set()
  const hubChecks = new Map() // sessionId -> { lastCheckAt, baselineUserSeq }
  ...
  function onStatus(payload) {
    if (payload !== null && payload !== undefined && payload.status === 'idle') {
      tick(payload.agent)
      maybePaceCheck(payload.agent)
      void maybeHubCheck(payload.agent)
    }
  }
  ...
  /**
   * v27: idle-hub re-activation. A hub with spokes that stays silent for
   * `hubCheckMinutes` gets one light "Coordinator check-in" steer. Any user
   * message (or forwarded reply) in the hub session resets the window; a
   * busy agent is never interrupted. First observation only arms the window.
   * v31: the check-in also measures every spoke's context pressure and
   * surfaces spokes past the rotate threshold.
   */
  const checkingHubs = new Set()
  async function maybeHubCheck(agent) {
    if (agent === null || agent === undefined) return
    ensureLinksRestored(agent.session)
    const spokes = coordHubs.get(agent.id)
    if (spokes === undefined || spokes.size === 0) return
    if (agent.status === 'running') return
    if (checkingHubs.has(agent.id)) return
    const now = Date.now()
    const cache = syncCache(agent.session)
    const lastUser = cache.lastUserSeq
    let record = hubChecks.get(agent.id)
    if (record === undefined) {
      hubChecks.set(agent.id, { lastCheckAt: now, baselineUserSeq: lastUser })
      return
    }
    // Incoming activity (user message or forwarded reply) resets the window.
    if (lastUser !== record.baselineUserSeq) {
      record.baselineUserSeq = lastUser
      record.lastCheckAt = now
      hubChecks.set(agent.id, record)
      return
    }
    if (!hubCheckDue(record, cfg, now)) return
    const idleMin = Math.max(1, Math.round((now - record.lastCheckAt) / 60000))
    record.lastCheckAt = now
    hubChecks.set(agent.id, record)
    checkingHubs.add(agent.id)
    try {
      const alerts = await collectSpokePressureAlerts(agent)
      agent.steer(userMessage(hubCheckPrompt(idleMin, agent.id, alerts), 'continuity-hub-check'))
    } catch { /* best-effort */ } finally {
      checkingHubs.delete(agent.id)
    }
  }

  /** v31: per-spoke context pressure, same math as /continuity (best-effort). */
  async function collectSpokePressureAlerts(hubAgent) {
    const spokes = coordHubs.get(hubAgent.id)
    if (spokes === undefined || spokes.size === 0) return []
    const agentsSvc = ctx.get('agents')
    if (agentsSvc === undefined) return []
    const alerts = []
    for (const spokeId of [...spokes]) {
      const spokeAgent = agentsSvc.get(spokeId)
      if (spokeAgent === undefined || spokeAgent.session === undefined) {
        alerts.push(spokeId + ': not live (it may have ended — confirm its status)')
        continue
      }
      try {
        let totalTokens = null
        if (meter !== undefined) totalTokens = meter.measure(spokeAgent.session).totalTokens
        const capacity = await resolveCapacity(spokeAgent, spokeAgent.session)
        const alert = spokePressureAlert(spokeId, totalTokens, capacity, cfg.rotateRatio)
        if (alert !== null) alerts.push(alert)
      } catch { /* per-spoke best-effort */ }
    }
    return alerts
  }

  /** v27: every hub with links gets a due-ness check (driven by the tick below). */
  function hubTick() {
    const agentsSvc = ctx.get('agents')
    if (agentsSvc === undefined) return
    for (const hubId of [...coordHubs.keys()]) {
      const agent = agentsSvc.get(hubId)
      if (agent !== undefined) void maybeHubCheck(agent)
    }
  }
  ...
  async function handleMission(invocation) {
    try {
      ...
      if (raw === 'stop') {
        if (typeof mission.stop !== 'function') {
          return { kind: 'error', text: 'Mission driver does not support /mission stop (needs the host continuity-mission v7 row).' }
        }
        return mission.stop(invocation.agent)
      }
      if (raw === '') return { kind: 'error', text: 'Usage: /mission <goal> — or /mission status, /mission stop, /mission resume' }
  ...
  // v26: bare /status — alias of /mission status.
  async function handleStatus(invocation) {
    return await handleMissionStatus(invocation)
  }

  // ── coordination mode (v10): link two sessions; replies auto-forward ──────

  /**
   * v27: rebuild this session's coordination links from its newest durable
   * record (written by /coordinate and /coordinate-hub). Restore-once per
   * session per process generation — after a restart the log is the truth.
   */
  function restoreLinks(session) {
    ...
    for (let index = events.length - 1; index >= 0; index -= 1) {
      ...
      const text = textOfMessage(event.data && event.data.message)
      if (typeof text !== 'string' || !text.includes(COORD_LINK_MARKER)) continue
      const record = parseLinkRecord(text)
      if (record === null) continue
      // Newest record wins; one record per session is enough.
      if (record.peers !== undefined) {
        if (record.peers !== '') {
          const set = coordPeers(session.id)
          set.add(record.peers)
          coordLinks.set(session.id, set)
        }
      }
      if (record.hub !== undefined) {
        if (record.hub !== '' && record.hub !== session.id) {
          const set = coordPeers(session.id)
          set.add(record.hub)
          coordLinks.set(session.id, set)
        }
        if (Array.isArray(record.spokes)) {
          const spokeIds = record.spokes.filter((spoke) => spoke !== session.id)
          const set = coordHubs.get(session.id)
          if (set === undefined) coordHubs.set(session.id, new Set(spokeIds))
          else { for (const spoke of spokeIds) set.add(spoke) }
          for (const spoke of spokeIds) {
            const spokeSet = coordPeers(spoke)
            spokeSet.add(session.id)
            coordLinks.set(spoke, spokeSet)
          }
        }
      }
      return true
  ...
  /**
   * v28: full-log restore fallback — when the in-memory events no longer
   * carry the link record (e.g. compacted away), re-scan the durable log
   * via sessionQuery.readSession and rebuild the links for the next event.
   */
  async function restoreLinksFromStore(sessionId) {
    const sessionQuery = ctx.get('sessionQuery')
    if (sessionQuery === undefined || typeof sessionQuery.readSession !== 'function') return
    ...
    restoreLinks({ id: sessionId, events })
  }

  function ensureLinksRestored(session) {
    if (session === null || session === undefined || session.id === undefined) return
    if (restoredSessions.has(session.id)) return
    restoredSessions.add(session.id)
    if (!restoreLinks(session)) {
      void restoreLinksFromStore(session.id).catch(() => {})
    }
  }

  /** v27: inject the current in-memory link state as a durable record (best-effort). */
  function persistCurrentLinks(agent) {
    if (agent === null || agent === undefined || agent.session === undefined) return
    const id = agent.id
    const spokes = coordHubs.get(id)
    const peers = coordPeers(id)
    let record
    if (spokes !== undefined && spokes.size > 0) {
      record = COORD_LINK_MARKER + ' hub=' + id + ' spokes=' + [...spokes].join(',')
    } else if (peers.size > 0) {
      record = COORD_LINK_MARKER + ' peers=' + [...peers][0]
    } else {
      record = COORD_LINK_MARKER + ' hub='
    }
    try {
      agent.inject(userMessage(record, 'continuity-links'))
    } catch { /* best-effort */ }
  }
  ...
  async function handleCoordinateHub(invocation) {
    ...
    for (const spoke of tokens) {
      ...
      linked.push(spoke)
      // v27: durable records on both sides (hub record + spoke record).
      persistCurrentLinks(agent)
      persistCurrentLinks(agentsSvc.get(spoke))
    }
    ...
    try {
      agent.steer(userMessage(coordinatorOnboardingPrompt(agent.id, linked, note, cfg.coordinateForwardMarker), 'continuity-coord-hub'))
    ...
    return {
      ...
      text: 'Coordination hub active. Spokes (auto-forward to you): ' + linked.join(', ')
        ...
        + '- v27: hub links are durable (recovered automatically after a restart); after ' + String(cfg.hubCheckMinutes) + ' silent minutes the hub re-checks its spokes on its own.',
    }
  }
  ...
  async function handleCoordinate(invocation) {
    ...
    // v27: durable records so the link survives a restart.
    persistCurrentLinks(agent)
    persistCurrentLinks(target)
  ...
  async function handleRelay(invocation) {
    ensureLinksRestored(invocation.agent.session)
    const raw = (typeof invocation.rawInput === 'string' ? invocation.rawInput : '').trim()
    const split = raw.indexOf(' ')
  ...
  async function handleUncoordinate(invocation) {
    ...
    // v27: rewrite the durable records so the unlink also survives a restart.
    persistCurrentLinks(agent)
    const agentsSvc = ctx.get('agents')
    const target = agentsSvc !== undefined ? agentsSvc.get(raw) : undefined
    if (target !== undefined) persistCurrentLinks(target)
  ...
  function onCoordEvent(session, event) {
    ...
    // v27: rebuild this session's links from the durable log once (restart-proof).
    ensureLinksRestored(session)
    ...
    // v28: marker gate — when configured, only turn-final messages containing
    // the marker are forwarded; unmarked progress stays quiet and is covered
    // by the periodic hub check-in (no ack chatter, no context dilution).
    if (cfg.coordinateForwardMarker !== '' && !text.includes(cfg.coordinateForwardMarker)) {
      // v30: completion reports always pass — a finished spoke reaches the
      // hub even without the coordination marker.
      if (!text.includes(WORKER_REPORT_MARKER)) return
    }
  ...
  // v29/v30: the forward-marker protocol lives in the roles section too, so
  // every session (spokes included) knows it from its own system prompt — no
  // coordinator relay needed, applies automatically after a restart. v30:
  // finishing a task ALWAYS ends with the marker (or ## Worker report).
  const FORWARD_MARKER_ROLES_LINE = cfg.coordinateForwardMarker !== ''
    ? '- ALL ROLES — forward marker protocol (v29/v30): this deployment auto-forwards only messages containing "' + cfg.coordinateForwardMarker + '" (or "## Worker report"). Prefix "' + cfg.coordinateForwardMarker + '" to your FINAL reply when you want the coordinator to intervene. When you FINISH a task, ALWAYS end with "' + cfg.coordinateForwardMarker + '" (or "## Worker report") even if nothing needs coordination — otherwise the coordinator only notices at its next periodic check-in.'
    : null
  const CONTINUITY_ROLES_SECTION = [
    '## Orchestra roles（乐团模式）',
    ...(FORWARD_MARKER_ROLES_LINE === null ? [] : [FORWARD_MARKER_ROLES_LINE]),
  ...
  ctx.effect(() => {
    const disposers = []
    // v27: idle-hub check-in ticker — the host timer service drives it in
    // production (the web profile has one); without a timer service (unit
    // tests) the loop stays event-driven via agent/status, so a bare test
    // process never hangs on a live interval.
    let tickerDisposed = false
    const timerSvc = ctx.get('timer')
    if (timerSvc !== undefined && typeof timerSvc.timeout === 'function') {
      const loop = () => {
        if (tickerDisposed) return
        timerSvc.timeout(60000).then(() => {
          if (tickerDisposed) return
          try { hubTick() } catch { /* best-effort */ }
          loop()
        })
      }
      loop()
      disposers.push(() => { tickerDisposed = true })
    }
    ...
    if (commands !== undefined) {
      ...
      disposers.push(commands.register({
        name: 'mission',
        description: 'Run the bounded automatic mission loop over a high-level goal; show its status; or cancel it.',
        input: { hint: '<goal> | status | stop' },
      ...
      disposers.push(commands.register({
        name: 'status',
        description: 'Show the current mission phase and per-task progress (bare alias of /mission status).',
